package aaaa.work.北京弘海科技发展有限公司;
...
public class PWork11 {
	...
	public static void main(String[] args) {
		...
		String html = Template_P.getGoodsHtml("http://www.honghai-tech.com/content/?133.html");
		Document doc = Jsoup.parse(html);

		Elements elements = doc.select("div.content-left-nav a");
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			try {
				mainFuction(
						1, //1插入，2不插入
						"北京弘海科技发展有限公司",//公司名称
						"http://www.honghai-tech.com" + elements.get(i).attr("href"),//网站网址
						1,//页数
						"body",//标签的element集合
						"http://www.honghai-tech.com",//图片地址包含完整时为“”，不完整填 域名
						"class. content-right-content@@@p@@@img@@@src",//获取图片标签
						elements.get(i).text(),//获取商品名称
						"http://www.honghai-tech.com" + elements.get(i).attr("href"),//产品url完整时为“”，不完整填 域名
						""//产品标签
				);
			} catch (Exception e) {
				e.printStackTrace();
