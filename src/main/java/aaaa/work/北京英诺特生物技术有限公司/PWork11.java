package aaaa.work.北京英诺特生物技术有限公司;
...
public class PWork11 {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			try {
				mainFuction(
						...
						"body",//标签的element集合
						"",//图片地址包含完整时为“”，不完整填 域名
						"div.float_r er_cp_hxfgf_r@@@img@@@src",//获取图片标签
						"",//获取商品名称
	...
	public static void mainFuction(int state,String company,String searchURL,
		...
		Function<Element, String> FimgUrl = (e) -> {
			...
			String img = e.getElementsByAttributeValue("class","body02").select("img").get(2).attr("src");
			return imgurl + img;
		};
		...
		Function<Element, String> FgoodName = (e) ->{
			//li.on:eq(1)@@@a@@@text
			String name = e.select("div.er_l_li li.on a").text();
			if (name.equals("")){
				name = e.select("div.er_cp_hxfgf_title").text();
			}
			if (name.equals("")){
				name = e.select("div.er_gy_title").text();
			}
			return name;
		};
